...
function ShopBanner({ sales = 0, owner, user_id }) {
            ...
            .then((response) => {
                ...
                if (response.status === 200) {
                    ...
                    if (response.data[0].shop_image != null)
                        setShopImage(
                            `${BASE_URL}/images/${response.data[0].shop_image}`
                        );
                    if (response.data[0].user_image != null)
                        setProfileImg(
                            `${BASE_URL}/images/${response.data[0].user_image}`
                        );
                }
            })
            .catch((err) => {
        ...
        axios
            ...
            .then((response) => {
                ...
                if (response.status === 200) {
                    ...
                    // setUpdated(true);
                    setShopImage(
                        `${BASE_URL}/images/${response.data.imageName}`
                    );
                }
            })
            .catch((err) => {
    ...
    return (
        ...
        /> */}
                ...
                <div style={{ display: "flex", alignItems: "space-between" }}>
                    {owner && (
                        <>
                            ...
                            />
                            <p className="shopBanner_text_sales">
                                {" "}
                                {sales} sales
                            </p>
                        </>
            ...
            >
                <img
                    className="shopBanner__main_img"
                    src={profileImg}
                    alt={name}
                />
